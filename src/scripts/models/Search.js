...
export default class Search {
  ...
  createSearchTag(searchDetails) {
    if (searchDetails.input !== '') {
      ...
      // On "bind(this)" afin de conserver la référence sur l'object (dans this) et d'éviter qu'il soit écraser par l'event
  ...
  closeSearchTag(event) {
    ...
    if (tagRelatedSearch) {
      ...
      this.searchHistory.forEach(search => {
        // On "reset" l'input afin d'éviter de créer un tag
        search.input = "";
        this.displayResults(search);
      });

      // TODO : faire la condition si le tableau est vide afin de ré-afficher toutes les recettes
    };
  };
